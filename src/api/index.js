...
export async function toc (config) {
  ...
  const toc_instance = axios.create({
    timeout: 10 * 1000,
    baseURL: process.env.NODE_ENV === 'production' ? `https://wechatx.34580.com/${cityFlag}` : `/${cityFlag}`,
    headers: {
      'content-type': 'application/json;charset=utf-8',
      'accept': 'application/json'
    },
    ...
    if (data.Error === 0 || !data.Error) return data
      ...
      if (data.Error !== void 0) {
        if (data.Error === -1) {
          ...
          setTimeout(() => {
            SignInMart.init({ test: process.env.NODE_ENV !== 'production' }).execute()
          }, 2000)
